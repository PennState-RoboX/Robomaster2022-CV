...
def main(camera: CameraSource, target_color: TargetColor):
    ...
    while True:
        ...
        if success:
            ...
            cartesian_pos = spherical_to_cartesian(
                Yaw, Pitch, depth) - np.array(camera.active_cam_config['camera_offset'])

            # 将欧拉角转换为弧度
            imu_yaw, imu_pitch, imu_roll = np.radians([imu_yaw, imu_pitch, imu_roll])
            print("imu: ",imu_yaw, imu_pitch)
            # 定义旋转矩阵
            R_yaw = np.array([
                [np.cos(imu_yaw), -np.sin(imu_yaw), 0],
                [np.sin(imu_yaw), np.cos(imu_yaw), 0],
                [0, 0, 1]
            ])
            R_pitch = np.array([
                [np.cos(imu_pitch), 0, np.sin(imu_pitch)],
                [0, 1, 0],
                [-np.sin(imu_pitch), 0, np.cos(imu_pitch)]
            ])
            R_roll = np.array([
                [1, 0, 0],
                [0, np.cos(imu_roll), -np.sin(imu_roll)],
                [0, np.sin(imu_roll), np.cos(imu_roll)]
            ])

            # 计算总的旋转矩阵
            R = np.dot(R_roll, np.dot(R_pitch, R_yaw))

            # 将旋转矩阵应用到cartesian_pos(x,y,z)
            rotated_cartesian_pos = np.dot(R, cartesian_pos)

            print(rotated_cartesian_pos)
            ...
            if (-30 < Pitch < 30) and (-45 < Yaw < 45):
                ...
                cv2.putText(frame, str(rotated_cartesian_pos[0]), (90, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0, 255, 0])
                cv2.putText(frame, str(rotated_cartesian_pos[1]), (90, 80),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0, 255, 0])
                ...
                if len(target_angle_history) < 1 or current_time - target_angle_history[-1][0] > max_history_frame_delta:
                    target_angle_history = [(current_time, *rotated_cartesian_pos)]
                else:
                    target_angle_history.append((current_time, *rotated_cartesian_pos))
                ...
                if len(target_angle_history) >= 2:
                    ...
                    predicted_yaw, predicted_pitch = rotated_cartesian_pos[0], rotated_cartesian_pos[1]
